...
*Last updated 2026-05-07. For game design and mechanics, see [GAME_SPEC.md](./GAME_SPEC.md).*
...
**Verification contract.** A phase is "Done" only when **both** of the following are confirmed and recorded:
1. Vitest output for that phase showing the red → green transition (paste or link the run log when closing the phase PR).
2. The Manual Verify Walk-through executed end-to-end in the browser (not "the tests passed, so the feature works"). Type-checking and unit tests verify code correctness; only the walk-through verifies feature correctness.

If either is missing, the phase is not Done and the next phase does not start.
...
- Crafting (Bakery, Larder, Forge recipes — 12 of 12 recipes registered in `constants.js`; 0 have Vitest coverage prior to Phase 10.3/10.9 — Phase 10.3 covers the 4 new late-game recipes and Phase 10.9 backfills the 8 pre-existing ones)
...
- Vitest installed (`vitest@^4.1.5` in `package.json`) with one existing reducer test — Phase 12 completes the migration of in-game `runSelfTests()` assertions into per-phase Vitest files
...
- Reshuffle Horn is awarded (daily streak / almanac) and present in `tools.shuffle`, but `USE_TOOL` only fires a bubble — the actual `shuffleBoard()` call and 360° spin animation are not wired (Phase 1.6 fixes)
- Season effects are partially wired inline in `state.js` but not centralized in a `SEASON_EFFECTS` constant; winter min-chain is `< 4` in code (`state.js:186`) vs §6 spec `5+` (Phase 0.13 fixes)
- Workers are hired but have no effect on gameplay; coin-only hire cost contradicts §18 lock (Phase 4.6 fixes via generic Worker pool)
- Granary's §11 "increases inventory cap" effect is inert — no cap mechanism exists (Phase 4.7 introduces it)
...
- No Influence currency or decoration buildings (Phase 8.5 adds)
- No Magic Portal summons or magic tools (Phase 8.6 adds Wand/Hourglass/Magic Seed/Magic Fertilizer)
- Mine hazard set incomplete: cave-ins + gas vents only; no lava (Phase 9.5) or moles (Phase 9.6)
- Mine entry middle tier (`100◉ + 10 shovels`) per §7 is missing — Phase 3 covers free + premium only (Phase 3.6 adds the middle tier and a Workshop-craftable Shovel)
- Field-state saving (Silos/Barns preserve tile layout per §18 LOCKED) is unimplemented — Silos and Barns are absent from §11 building list (Phase 12.5 introduces them and the save-shape change)
- Farm hazard set incomplete: no rats counter tool (Phase 10.5 Cat), no Fire (10.7), no Wolves (10.8)
- Priority Farm tools incomplete: no Bird Cage / full Scythe / Cat / Rifle / Hound (Phase 10.5/10.6/10.8 adds)
...
13 small fixes that must land before any feature work. Removes silent data corruption, dead-code mechanics, and constant mismatches (grid 6×6, MAX_TURNS=10, mood ordering bug, NPC bond floor, Ember Drake forge gating, drought/frost effects, dev menu gating, plus 0.13 — centralise the four §6 season effects into a single `SEASON_EFFECTS` constant and fix winter min-chain from `< 4` to `5+`). Exit: `runSelfTests()` returns zero failures and a fresh game shows the spec'd starting conditions.

### Phase 1 — Chain Mechanic Overhaul + Board Tools
Replaces the global "every 3rd tile" upgrade rule with per-resource thresholds (hay→wheat at 6, grain→flour at 4, etc.). Adds endpoint upgrade spawning, escalating star markers (1×/2×/3×), dead-board auto-shuffle, three working tools (Scythe / Seedpack / Lockbox) that visibly modify the board instead of silently adding inventory, and 1.6 — wires the existing `tools.shuffle` slot through `shuffleBoard()` with the §5-spec'd full 360° spin animation so the Reshuffle Horn awarded by Phase 3.5 / Phase 7.2 actually does something on press. Exit: 10 sessions played, no softlocks, every upgrade lands at the chain endpoint, Reshuffle Horn spins the board.
...
### Phase 8 — Boss + Weather + Magic Portal
Makes the §18-locked 1-season (10-turn) boss window actually *feel* different from a normal season. Each of the four canonical bosses applies a visible board modifier — Frostmaw freezes 2 columns (no chains through them), Ember Drake spawns heat tiles that burn an inventory unit if not chained within 2 turns, Old Stoneface drops 4 rubble blocks that only clear via adjacent 3+ stone chains, and Mossback (year-2 spring alternate) hides ~4 mystery tiles that flip on chain. Reward replaces Phase 0's flat ±200◉ with a two-piece formula: base = `200◉ × year_number` (locked §9), plus a margin-of-victory bonus that pays half-margin scaling capped at 2× target — so 60 logs vs Frostmaw's 30 pays 1.5× the year base, while 30 exactly pays 1.0×, and 29 pays nothing (no flag flip, no rune). The non-boss seasons get a small weather banner rolled from a 100-weight table (40 None / 20 Rain / 15 Harvest Moon / 15 Drought / 10 Frost): rain doubles berry yields, harvest moon adds +1 upgrade per chain, drought halves wheat+grain spawn weights, frost slows the collapse tween (purely visual). Boss seasons skip the weather roll. §8.5 introduces the §3-locked Influence currency and three repeatable decoration buildings (Violet Bed +20, Stone Lantern +35, Apple Sapling +60) that are the only Influence source until Phase 7 wires royal-quest Influence rewards. §8.6 wires the Magic Portal summon UI (built in Phase 3.3) and the four §5 magic tools — Magic Wand (80), Hourglass (120), Magic Seed (100), Magic Fertilizer (60) — each player-picked per §18, each respecting the locked no-turn-cost contract. Exit: a fresh save can play through year 1 → 2 hitting all 4 boss types, see margin-scaled coin payouts, save+reload mid-boss without losing modifier state, observe weather roll only during non-boss seasons, build decorations to stockpile 200 Influence, and summon Magic Wand + Magic Seed at the Portal.

### Phase 9 — Mine biome
Opens the second biome the moment Phase 2's `act3_mine_opened` beat fires and Phase 3's supply-or-rune entry fee is paid. The board palette flips from warm Farm green to cool cavern grey, and the spawn pool swaps to a locked 9-slot mix of stone / ore / coal / dirt + rare gem feeding the Phase 1 thresholds (stone→cobble at 8, cobble→block at 6, ore→ingot at 6, coal→coke at 7, gem→cutgem at 5; ingot and gold terminal). Once per mine session a Mysterious Ore tile spawns with a 5-turn countdown — chaining it together with ≥ 2 Dirt tiles yields exactly +1 Rune (Phase 3 currency); letting it expire degrades it to plain Dirt with no reward. All four §7 hazards now ship: cave-ins lock a row behind rubble until cleared by a 3+ stone chain in an adjacent row (9.3), gas vents glow on a 2×2 region for 3 turns then steal the next turn if ignored (9.3), lava spreads one orthogonal cell per turn destroying tiles and is converted to rubble by the new Workshop-craftable Water Pump (9.5), and moles consume one adjacent tile per turn and hop on timer-zero — cleared instantly by Workshop-craftable Explosives, which also clears any active cave-in (9.6). Single-active hazard cap unchanged (5% per-fillBoard); pool weights cave_in 25 / gas_vent 40 / lava 20 / mole 15. Two new mine-only workers extend the Phase 4 catalog under its locked max-effect model — Canary (max 2 → -50% gas vent spawn rate at full hire) and Geologist (max 2 → +1 ore + +1 gem pool weight at full hire) — both gated by Phase 4.5 shared housing capacity. Exit: a fresh save can fire the mine beat, pay the entry fee, switch biomes, capture a Mysterious Ore for a Rune, clear a cave-in, survive a lava outbreak with a Water Pump, exterminate a mole with Explosives, and hire both mine workers without breaking the Farm side of the game.

### Phase 10 — Farm depth
Polishes the Farm to high-value before any further Mine work, honouring the locked Farm-first priority order. Three new Workshop-craftable tools — Rake (clears all hay tiles, 1 plank), Axe (clears all log tiles, 1 stone), Fertilizer (next refill spawns all grain, 1 hay + 1 dirt) — extend the Phase 1 board-tool tray under the same locked no-turn-cost contract, animating directly on the canvas instead of editing inventory in the dark. A per-season tile-pool modifier layer (spring berry +1, summer wheat +1, autumn log +2, winter stone +1 / hay −1) sits on top of the §6 base pool and stacks additively with Phase 4 worker `pool_weight` and Phase 5 species filtering — without touching the four locked §6 season effects (harvest +20%, summer 2× coins, autumn 2× upgrades, winter min-chain-5). Four §11 late-game Forge recipes (Iron Frame, Stonework, Gem Crown, Gold Ring) close out the Mine → Forge → Market loop and become Phase 7-quest-targetable outputs. All three §6 Farm hazards now ship: Rats are active when hay > 50 AND wheat > 50 with chain-3-to-clear (10.4) backed by the §6 Cat tool that clears them in one tap (10.5); Bird Cage and full Scythe round out the §5 priority Farm tools (10.6); Fire seeds at 4% per fillBoard, spreads at 50% per turn, and is extinguished by routing chains through it (+2◉ per fire, 10.7); Wolves spawn at 6% when birds > 30, eat adjacent bird tiles, and are countered by Rifle (clear) or Hound (5-turn scatter) (10.8). 10.9 backfills Vitest coverage for **all eight** pre-existing crafting recipes (Bread Loaf, Honey Roll, Harvest Pie, Preserve, Tincture, Iron Hinge, Cobble Path, Lantern) that have lived in `constants.js` since Phase 0 with no test coverage — plus the four Phase 10.3 late-game recipes for an end-of-Phase-10 total of 12/12 §11 recipes under Vitest validation. Exit: a fresh save can craft and use all eight Farm tools without spending a turn, eyeball-distinguish a spring board from a winter board, forge and sell all four late-game items via Phase 3 Market, survive every §6 hazard with the right tool, and pass `npm test -- preexisting-recipes` clean.
...
*Completes* the Vitest migration (the runner is already installed in `package.json` and `src/__tests__/reducers.test.js` exists). Every `runSelfTests()` assertion from Phases 0–11 ports into per-phase Vitest files under `tests/phase-N-*.test.js`, with a coverage gate of 70% on `src/utils.js`, `src/state.js`, `src/features/**`. The in-game self-test path stays callable as a sub-50ms smoke shim against cross-phase invariants (MAX_TURNS, fresh bonds, initial story beat). A versioned save-migration pipeline (`SAVE_SCHEMA_VERSION = 12`, one pure idempotent step per Phase 1..12 save-shape change) lets a v0 save from a year ago load cleanly into the Phase 12 build with every later slice (story, market, workers, species, bonds, quests, weather, mine hazards, farm tools, prefs, field-state) initialised to spec'd defaults — with explicit fallback-to-fresh on corrupted saves. 12.5 finally honours the §18 LOCKED "Silos/Barns preserve tile layout between sessions" line: extends `src/constants.js` BUILDINGS with `silo` (200◉ + 15 plank, lv 4) and `barn` (350◉ + 25 plank + 5 stone, lv 5), wires a per-biome `state.<biome>.savedField` snapshot taken on `CLOSE_SEASON` and restored on next biome entry only when the building is built, and ships its own save-migration step. A GitHub Actions workflow runs `lint` + `test` + `build` in parallel on every PR against `main` (Node 20, npm cache, <3 minute target, all three required for merge). Vite build extends with manual chunking (`vendor/phaser`, `vendor/react`), gzip + brotli pre-compression, `rollup-plugin-visualizer` writing `dist/stats.html`, and preload hints — targeting <200KB gzipped main entry and <500KB total first-load. Exit: a v0 save fixture round-trips to v12 with no data loss, a deliberate chain-math regression fails CI in <2 minutes, building a Silo and ending a season then re-entering Farm produces the *exact same* tile layout (no auto-shuffle), and the production bundle hits the size budget.
