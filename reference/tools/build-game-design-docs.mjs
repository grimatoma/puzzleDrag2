...
${STACK_SVG}
    ...
    <section id="shape">
      ...
      <p><b>The long arc</b> is a <b>map of seven towns</b>, all required for the finale but openable in any order the map allows. You found each by caravan, build it up, and beat its signature task — but you need not finish one before opening the next: <b>several towns can be in progress at once and you switch between them freely</b>. Difficulty is <b>cozy early, ramping late</b>: the early sites are forgiving, and the rising founding cost plus a gentle progress threshold keep the harder ones a nudge out of reach until you have some footing. <b>Workers, coins, tools and Villagers travel with you</b>; each town's materials are its own.</p>
    </section>
...
const towns = `  <div class="panel" id="path" role="tabpanel" aria-label="The path">
    <section id="founding">
      ...
      <p>You open a new town by <b>outfitting a caravan</b> and rolling it out to a site on the map. The caravan is paid in <b>portable goods — coins plus a small basket of staples you can make in any town you already run</b> (rations, planks), never the target town's own materials. So founding never depends on a place you have not opened yet: anything the caravan needs, you can already produce.</p>
      <p>Founding is <b>non-linear</b>. You can open any town the map lets you reach (a site adjacent to one you have already settled) as soon as you can afford it — you do <b>not</b> have to finish the town you are in first. Several towns can be <b>in progress at once</b>, and you <b>switch between them freely</b>; if a town is not grabbing you, leave it and develop another, then come back. The caravan's coin cost <b>rises with each town you open</b>, and the hardest sites sit behind a gentle progress threshold, so the map still unfurls at a cozy pace — a nudge toward an order, never a wall. Home is where you start, auto-founded for free; every town after is caravan-founded.</p>
    </section>
    <section id="arc">
    ...
    <section id="t1">
      <div class="town farm">
        ...
        <ol class="play">
          ...
          <li><b>Build the caravan.</b> Gather the staple basket, outfit it at the Caravan Yard, and roll out to your first new site (Two Rivers sits just over the ridge).</li>
        </ol>
      </div>
    </section>
    <section id="t2">
    ...
    <section id="intro">
      ...
      <table id="status-table">
        ...
        <tbody>
          ...
          <tr data-status="build"><td><b>Light soft availability gate</b></td><td><span class="pill warn">build</span></td><td>Replace the hard <code>requiresZoneTier</code> prereqs (<code>src/features/zones/data.ts:442</code>) with a gentle progress threshold (coins-earned / buildings-raised) on the hardest sites — a nudge, not a wall. The rising founding coin cost stays as the main soft brake.</td></tr>
          <tr data-status="wire"><td><b>Caravan = the founding act (portable cost)</b></td><td><span class="pill info">half-wired</span></td><td>Founding is a coin dispatch today and <code>caravan_post</code> is the market/sell building. Surface founding as "outfit a caravan" paid in <b>coins + portable staples</b> (rations/planks) — never the target town's own materials (the no-softlock rule).</td></tr>
