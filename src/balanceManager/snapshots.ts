...
export function readSnapshots(storage?: Storage | null): Record<string, { savedAt: string | null; version: number; draft: unknown }> {
  ...
  try {
    ...
    const out: Record<string, { savedAt: string | null; version: number; draft: unknown }> = {};
    for (const [name, rawEntry] of Object.entries(parsed)) {
      ...
      out[name] = {
        ...
        version: Number.isFinite(entry["version"]) ? (entry["version"] as number) : SNAPSHOT_VERSION,
        draft: entry["draft"],
      };
    }
    return out;
