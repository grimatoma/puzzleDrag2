...
function hooksToAbilities(hooks: any) {
...
export function writeBalanceDraft(draft: any) {
  try {
...
export function mergeOverrides(a: any, b: any) {
  if (!a) return b || {};
  if (!b) return a || {};
  const out = { ...a };
...
export function applyUpgradeThresholdOverrides(target: any, overrides: any) {
 ...
 */
export function applyItemOverrides(items: any, overrides: any) {
  if (!overrides || typeof overrides !== "object") return;
  for (const [key, patch] of Object.entries(overrides) as [string, any][]) {
...
export function applyRecipeOverrides(recipes: any, overrides: any) {
  if (!overrides || typeof overrides !== "object") return;
  for (const [key, patch] of Object.entries(overrides) as [string, any][]) {
    const r = recipes[key];
    if (!r) continue;
...
export function applyBuildingOverrides(buildings: any, overrides: any) {
  if (!overrides || typeof overrides !== "object") return;
  for (const b of buildings) {
    ...
    if (Array.isArray(patch.abilities)) {
      b.abilities = patch.abilities.filter((a: any) => a && typeof a === "object" && typeof a.id === "string");
    }
  }
}
 ...
 */
export function applyTileOverrides(tileTypes: any, overrides: any) {
    ...
    if (powerPatch) {
      ...
      if (Array.isArray(powerPatch.abilities)) {
        newAbilities = powerPatch.abilities.filter(
          (a: any) => a && typeof a === "object" && typeof a.id === "string",
        );
 ...
 */
export function applyZoneOverrides(zones: any, overrides: any) {
  if (!overrides || typeof overrides !== "object") return;
  for (const [zoneId, patch] of Object.entries(overrides) as [string, any][]) {
 ...
 */
export function applyWorkerOverrides(workers: any, overrides: any) {
 ...
 */
export function applyKeeperOverrides(keepers: any, overrides: any) {
  if (!keepers || !overrides || typeof overrides !== "object") return;
  const strArray = (v: any) => (Array.isArray(v) ? v.map((x: any) => String(x)).filter((s: string) => s.length > 0) : null);
  const patchPath = (target: any, patch: any) => {
  ...
  for (const [type, patch] of Object.entries(overrides) as [string, any][]) {
 ...
 */
export function sanitizeTuning(o: any) {
  const out: any = {};
  if (!o || typeof o !== "object") return out;
  const posInt = (v: any) => (Number.isFinite(Number(v)) && Number(v) >= 1 ? Math.floor(Number(v)) : undefined);
  const intFields = {
 ...
 */
export function applyNpcOverrides(npcData: any, bondBands: any, overrides: any) {
  if (!overrides || typeof overrides !== "object") return;
  const strArr = (v: any) => (Array.isArray(v) ? v.filter((x: any) => typeof x === "string" && x.length > 0) : null);
  if (overrides.byId && typeof overrides.byId === "object" && npcData) {
    for (const [id, patch] of Object.entries(overrides.byId) as [string, any][]) {
      const d = npcData[id];
 ...
 */
export function sanitizeChoiceOutcome(raw: any) {
...
export function sanitizeChoiceArray(raw: any) {
  ...
  raw.forEach((c: any, i: number) => {
    ...
    seen.add(id);
    const choice: any = { id, label: (typeof c.label === "string" && c.label.length > 0) ? c.label : "Continue" };
 ...
 */
export function applyStoryOverrides(storyBeats: any, sideBeats: any, overrides: any) {
  if (!overrides || typeof overrides !== "object") return;
  ...
  if (Array.isArray(overrides.newBeats)) {
    ...
    for (const raw of overrides.newBeats) {
      ...
      taken.add(id);
      const beat: any = { id, side: true, draft: true };
  ...
  if (overrides.beats && typeof overrides.beats === "object") {
    ...
    for (const [beatId, patch] of Object.entries(overrides.beats) as [string, any][]) {
      ...
      if (Array.isArray(patch.choices)) {
        ...
        for (const [choiceId, cp] of Object.entries(patch.choices) as [string, any][]) {
