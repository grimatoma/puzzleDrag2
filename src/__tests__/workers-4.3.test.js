...
describe("Phase 4.3 — effective thresholds and pool weights", () => {
  ...
  it("no Hilda → hay threshold 6, 3-chain = 0 upgrades", () => {
    ...
    expect(t0.hay).toBe(6);
    expect(upgradeCountForChain(3, "hay", t0)).toBe(0);
  });
  ...
  it("3 Hilda → hay threshold 3, 3-chain = 1 upgrade", () => {
    ...
    expect(t1.hay).toBe(3);
    expect(upgradeCountForChain(3, "hay", t1)).toBe(1);
    expect(upgradeCountForChain(6, "hay", t1)).toBe(2);
  });

  it("threshold floors at 1 with massive reduce", () => {
    const synth = { thresholdReduce: { hay: 99 }, poolWeight:{}, bonusYield:{}, seasonBonus:{} };
    expect(effThresholds(synth).hay).toBe(1);
  });
  ...
  it("full Wila → +2 jam/chain bonus yield", () => {
    ...
    expect(computeWorkerEffects(fullW).bonusYield.jam).toBe(2);
  });

  it("upgradeCountForChain falls back to base UPGRADE_THRESHOLDS when no map passed", () => {
    expect(upgradeCountForChain(6, "hay")).toBe(1);
  });
});
