...
describe("runSummary slice — chain accumulation", () => {
  ...
  it("accumulates chains, picks biggest, totals coins/upgrades", () => {
    ...
    s = reduce(s, { type: "CHAIN_COLLECTED", payload: { key: "tile_grass_grass", gained: 4, upgrades: 0, chainLength: 4, value: 2 } });
  ...
  it("CLOSE empties the accumulator", () => {
    let s = reduce(baseState(), { type: "FARM/ENTER" });
    s = reduce(s, { type: "CHAIN_COLLECTED", payload: { key: "tile_grass_grass", gained: 3, upgrades: 0, chainLength: 3, value: 2 } });
  ...
  it("ignores tool-only (noTurn) chain emissions", () => {
    let s = reduce(baseState(), { type: "FARM/ENTER" });
    s = reduce(s, { type: "CHAIN_COLLECTED", payload: { key: "tile_grass_grass", gained: 5, upgrades: 0, chainLength: 5, value: 2, noTurn: true } });
    expect(s.runSummary.chainsPlayed).toBe(0);
  });
