...
// tier may extend session turns and/or skip a resource cost.

import { describe, it, expect } from "vitest";
import { rootReducer, createInitialState } from "../state.js";
import { HARBOR_ENTRY_TIERS, MAX_TURNS } from "../constants.js";
...
describe("HARBOR/ENTER tiers", () => {
  ...
  it("rejects free tier without enough wood_plank", () => {
    const s = { ...createInitialState(), inventory: { wood_plank: 1 } };
  ...
  it("free tier deducts wood_plank and switches to fish biome with default turns", () => {
    const s = { ...createInitialState(), inventory: { wood_plank: 5 } };
    const r = rootReducer(s, { type: "HARBOR/ENTER", payload: { tier: "free" } });
  ...
  it("better tier rejects without coins", () => {
    const s = { ...createInitialState(), coins: 100, inventory: { wood_plank: 5 } };
    const r = rootReducer(s, { type: "HARBOR/ENTER", payload: { tier: "better" } });
    expect(r).toBe(s);
  });

  it("better tier rejects without enough wood_plank", () => {
    const s = { ...createInitialState(), coins: 500, inventory: { wood_plank: 1 } };
    const r = rootReducer(s, { type: "HARBOR/ENTER", payload: { tier: "better" } });
    expect(r).toBe(s);
  });

  it("better tier deducts coins + planks and gives +2 sessionMaxTurns", () => {
    const s = { ...createInitialState(), coins: 500, inventory: { wood_plank: 10 } };
    const r = rootReducer(s, { type: "HARBOR/ENTER", payload: { tier: "better" } });
    expect(r.biomeKey).toBe("fish");
