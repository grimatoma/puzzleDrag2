...
describe("applyStoryOverrides", () => {
  ...
  it("trigger + repeat are sanitised on newBeats and beats[] patches", () => {
    ...
    applyStoryOverrides(story, side, {
      ...
      beats: { a1: { trigger: { type: "resource_total", key: "tree_oak", amount: 30 }, repeat: true },
               d1: { repeat: false, repeatCooldown: 0 } },   // turn the repeat back off
    });
    expect(story[0].trigger).toEqual({ type: "resource_total", key: "tree_oak", amount: 30 });
...
describe("flag-trigger sanitizers", () => {
  it("sanitizeFlagTrigger accepts the full event vocabulary, rejects junk", () => {
    ...
    expect(sanitizeFlagTrigger({ type: "resource_total", key: " tree_oak ", amount: 30.9 })).toEqual({ type: "resource_total", key: "tree_oak", amount: 30 });
...
describe("applyBossOverrides", () => {
  it("patches name/season/descriptions/target.amount; modifier untouched", () => {
    const bosses = [{ id: "frostmaw", name: "Frostmaw", season: "winter", target: { resource: "tree_oak", amount: 30 }, modifier: { type: "freeze_columns", params: { n: 2 } }, description: "old", modifierDescription: "old mod" }];
    ...
    expect(bosses[0].target).toEqual({ resource: "tree_oak", amount: 45 });
