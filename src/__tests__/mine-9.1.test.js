...
describe("Phase 9.1 — Stone/ore/coal/ingot resource chain + Mine biome setup", () => {
  ...
  it("MINE_TILE_POOL contains all mine base resources (iron + copper ore split)", () => {
    ...
    for (const k of ["tile_mine_stone", "tile_mine_iron_ore", "tile_mine_copper_ore", "tile_mine_coal", "tile_special_dirt", "tile_mine_gem"]) {
      expect(baseSet.has(k)).toBe(true);
    }
  });

  it("stone weighted ×3", () => {
    expect(MINE_TILE_POOL.filter((k) => k === "tile_mine_stone").length).toBe(3);
  });

  it("dirt weighted ×2", () => {
    expect(MINE_TILE_POOL.filter((k) => k === "tile_special_dirt").length).toBe(2);
  });

  it("gem weighted ×1 (rare)", () => {
    expect(MINE_TILE_POOL.filter((k) => k === "tile_mine_gem").length).toBe(1);
  });
  ...
  it("coal → coke at 7", () => {
    expect(UPGRADE_THRESHOLDS.tile_mine_coal).toBe(7);
  });
  it("gem → cut_gem at 5", () => {
    expect(UPGRADE_THRESHOLDS.tile_mine_gem).toBe(5);
  });
  it("gold → gold_bar at 6 (tile_mine_gold is now a normal mine family)", () => {
    expect(UPGRADE_THRESHOLDS.tile_mine_gold).toBe(6);
  });
  ...
  it("8 stone = 1 block upgrade", () => {
    expect(upgradeCountForChain(8, "tile_mine_stone")).toBe(1);
  });
  it("16 stone = 2 block upgrades", () => {
    expect(upgradeCountForChain(16, "tile_mine_stone")).toBe(2);
  });
  it("6 iron_ore = 1 iron_bar", () => {
    expect(upgradeCountForChain(6, "tile_mine_iron_ore")).toBe(1);
  });
  it("6 copper_ore = 1 copper_bar", () => {
    expect(upgradeCountForChain(6, "tile_mine_copper_ore")).toBe(1);
  });
  it("7 coal = 1 coke", () => {
    expect(upgradeCountForChain(7, "tile_mine_coal")).toBe(1);
  });
  it("5 gem = 1 cut_gem", () => {
