...
describe("castle slice — coverage gaps", () => {
  ...
  it("non-CASTLE/CONTRIBUTE action returns state unchanged", () => {
    const s0 = baseState();
    expect(castleReduce(s0, testAction({ type: "NOPE" }))).toBe(s0);
  });
  ...
  it("CASTLE/CONTRIBUTE with non-positive amount is rejected", () => {
    ...
    const s1 = castleReduce(s0, {
      type: "CASTLE/CONTRIBUTE",
      payload: { key: "soup", amount: 0 },
    } as Action);
    const s2 = castleReduce(s0, {
      type: "CASTLE/CONTRIBUTE",
  ...
  it("CASTLE/CONTRIBUTE deducts inventory and increments contributed (coal via tile_mine_coal)", () => {
    const s0 = baseState();
    const s1 = castleReduce(s0, {
      type: "CASTLE/CONTRIBUTE",
...
describe("story slice — coverage gaps", () => {
  ...
  it("STORY/DISMISS_MODAL pops to the next queued beat", () => {
    ...
    const s1 = storyReduce(s0, { type: "STORY/DISMISS_MODAL" } as Action);
  ...
  it("DEV/RESET_GAME is a story-side no-op (handled in core)", () => {
    const s0 = baseState();
    expect(storyReduce(s0, { type: "DEV/RESET_GAME" } as unknown as import("../types/state.js").Action)).toBe(s0);
  });

  it("unknown action returns state unchanged", () => {
    const s0 = baseState();
    expect(storyReduce(s0, testAction({ type: "NOPE" }))).toBe(s0);
  });
});

// ─── tutorial/slice.js ──────────────────────────────────────────────────────
describe("tutorial slice — coverage gaps", () => {
  const baseTutorial = (over: Record<string, unknown> = {}) => ({ active: true, step: 0, seen: false, ...over });
  const baseState = (over: Record<string, unknown> = {}) =>
    mergeTestState({ tutorial: baseTutorial(), modal: "tutorial", ...over });

  it("returns state unchanged when the tutorial slice is missing", () => {
    const s0 = unsafeGameState({ foo: 1 });
    expect(tutorialReduce(s0, { type: "TUTORIAL/NEXT" } as unknown as import("../types/state.js").Action)).toBe(s0);
  });
  ...
  it("auto-starts on the very first action when not seen and no modal is open", () => {
    ...
    const s1 = tutorialReduce(s0, testAction({ type: "FOO" }));
  ...
  it("does NOT auto-start for SET_BIOME / ADVANCE_SEASON / @@INIT", () => {
    ...
    for (const type of ["@@INIT", "SET_BIOME", "ADVANCE_SEASON", "TUTORIAL/START"]) {
      const s1 = tutorialReduce(s0, testAction({ type }));
  ...
  it("TUTORIAL/START activates step 0 and opens the modal", () => {
    ...
    const s1 = tutorialReduce(s0, { type: "TUTORIAL/START" } as Action);
    expect(s1.tutorial.active).toBe(true);
  ...
  it("TUTORIAL/SKIP ends the tutorial regardless of step", () => {
    ...
    const s1 = tutorialReduce(s0, { type: "TUTORIAL/SKIP" } as Action);
    expect(s1.tutorial.active).toBe(false);
    expect(s1.tutorial.seen).toBe(true);
    expect(s1.modal).toBeNull();
  });
  ...
  it("TUTORIAL/PREV when not active is a no-op", () => {
    ...
    const s1 = tutorialReduce(s0, { type: "TUTORIAL/PREV" } as Action);
    expect(s1).toBe(s0);
  });
  ...
  it("unknown action while active and seen returns state unchanged", () => {
    ...
    const s1 = tutorialReduce(s0, testAction({ type: "TOTALLY_UNRELATED" }));
    expect(s1).toBe(s0);
  });
});
