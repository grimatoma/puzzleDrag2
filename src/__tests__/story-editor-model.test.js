...
describe("story editor preview model", () => {
  it("applies choice outcomes using runtime story semantics", () => {
    ...
    expect(next.resources.tree_oak).toBe(3);
  ...
  it("finds downstream beats unlocked by preview state", () => {
    const d = draftWith({
      newBeats: [
        ...
        { id: "resource_followup", title: "Resource", trigger: { type: "resource_total", key: "tree_oak", amount: 3 } },
      ],
    });
    ...
    const resourced = applyPreviewEffects(blankPreviewState(), null, { outcome: { resources: { tree_oak: 3 } } });
    expect(firstTriggeredByPreviewState(resourced, d, new Set(["source", "flag_followup"]))).toBe("resource_followup");
    expect(previewStateSummary(resourced).join(" ")).toContain("tree_oak");
  });
});
