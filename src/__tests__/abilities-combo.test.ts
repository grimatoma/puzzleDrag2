...
describe("Combination — building + worker + tile abilities stack on shared channels", () => {
  ...
  it("worker (fractional weight) + tile (full weight) on threshold_reduce stack on the same key", () => {
    const out = aggregateAbilities([
      { kind: "worker", abilities: [{ id: "threshold_reduce", params: { target: "tile_grass_grass", amount: 4 } }], weight: 0.5 },
      { kind: "tile", abilities: [{ id: "threshold_reduce", params: { target: "tile_grass_grass", amount: 1 } }], weight: 1 },
    ], ctx());
    expect(out.thresholdReduce.tile_grass_grass).toBe(3); // (4*0.5) + (1*1)
  });
});
