...
describe("10.4 — rollRatSpawn", () => {
  it("mine biome NEVER spawns rats (1000 rolls)", () => {
    const mine = {
      ...createInitialState(),
      biome: "mine",
      inventory: { home: { tile_grass_hay: 99, tile_grain_wheat: 99 } },
  ...
  it("hay exactly 50 (not > 50) does not spawn", () => {
    const s = {
      ...createInitialState(),
      biome: "farm",
      inventory: { home: { tile_grass_hay: 50, tile_grain_wheat: 51 } },
      hazards: { rats: [] },
      grid: makeGrid(),
    };
  ...
  it("hay > 50 AND wheat > 50 → spawns on low rng roll", () => {
    const s = {
      ...createInitialState(),
      biome: "farm",
      inventory: { home: { tile_grass_hay: 51, tile_grain_wheat: 51 } },
      hazards: { rats: [] },
      grid: makeGrid(),
    };
...
describe("10.4 — tickRats", () => {
  ...
  it("starving rat (no adjacent plant) — age still ticks, no tile eaten", () => {
    const s = {
      ...createInitialState(),
      biome: "farm",
      grid: [[{ key: "tile_mine_stone" }, { key: "rat" }, { key: "tile_mine_stone" }]],
