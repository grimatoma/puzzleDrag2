...
describe("7.3 distinct_resources_chained", () => {
  it("first chain of hay increments distinct counter", () => {
    const s = freshState();
    const r = tickAchievement(s, "distinct_resources_chained", 1, "tile_grass_hay");
    expect(r.newState.achievements.counters.distinct_resources_chained).toBe(1);
    expect(r.newState.achievements.seenResources.tile_grass_hay).toBe(true);
  });

  it("second chain of same resource does NOT increment distinct counter", () => {
    const s = freshState();
    const r1 = tickAchievement(s, "distinct_resources_chained", 1, "tile_grass_hay");
    const r2 = tickAchievement(r1.newState, "distinct_resources_chained", 1, "tile_grass_hay");
