...
describe("7.3 distinct_resources_chained", () => {
  it("first chain of hay increments distinct counter", () => {
    const s = freshState();
    const r = tickAchievement(s, "distinct_resources_chained", 1, "tile_grass_grass");
    expect(r.newState.achievements.counters.distinct_resources_chained).toBe(1);
    expect(r.newState.achievements.seenResources.tile_grass_grass).toBe(true);
  });

  it("second chain of same resource does NOT increment distinct counter", () => {
    const s = freshState();
    const r1 = tickAchievement(s, "distinct_resources_chained", 1, "tile_grass_grass");
    const r2 = tickAchievement(r1.newState, "distinct_resources_chained", 1, "tile_grass_grass");
  ...
  it("chaining a new resource (wheat) after hay increments to 2", () => {
    const s = freshState();
    const r1 = tickAchievement(s, "distinct_resources_chained", 1, "tile_grass_grass");
    const r2 = tickAchievement(r1.newState, "distinct_resources_chained", 1, "tile_grain_wheat");
