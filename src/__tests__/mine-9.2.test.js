...
describe("Phase 9.2 — Mysterious Ore", () => {
  ...
  it("tick at 1 → expire: ore cleared, no rune", () => {
    ...
    expect(expired.grid[t.mysteriousOre.row][t.mysteriousOre.col].key).toBe("tile_special_dirt");
  });

  // ── Chain validation ───────────────────────────────────────────────────────
  const ore = { key: "mysterious_ore", row: 2, col: 3 };
  const d1 = { key: "tile_special_dirt", row: 2, col: 4 };
  const d2 = { key: "tile_special_dirt", row: 2, col: 5 };
  const stone = { key: "tile_mine_stone", row: 2, col: 6 };
