...
describe("Phase 4.2 — computeWorkerEffects pure aggregator", () => {
  ...
  it("zero hires → all zeroes", () => {
    ...
    expect(r0.thresholdReduce.hay ?? 0).toBe(0);
    expect(r0.poolWeight.berry   ?? 0).toBe(0);
    expect(r0.bonusYield.jam     ?? 0).toBe(0);
  ...
  it("per-hire walk on Hilda (max-effect ÷ maxCount)", () => {
    ...
    expect(computeWorkerEffects(withHilda(1)).thresholdReduce.hay).toBe(1);
    expect(computeWorkerEffects(withHilda(2)).thresholdReduce.hay).toBe(2);
    expect(computeWorkerEffects(withHilda(3)).thresholdReduce.hay).toBe(3);
  });
  ...
  it("stacking across types", () => {
    ...
    expect(rB.thresholdReduce.hay).toBe(3);
  ...
  it("additive same-key stacking — synthetic worker map", () => {
    const synthMap = {
      ...WORKER_MAP,
      ghost: { id:"ghost", maxCount:2, effect: { type:"threshold_reduce", key:"hay", from:4, to:2 } }
    };
    const synthState = { workers: { hired: { hilda: 3, ghost: 2 }, debt: 0 } };
    expect(computeWorkerEffects(synthState, synthMap).thresholdReduce.hay).toBe(5);
  });
  ...
  it("debt > 0 suppresses ALL effects", () => {
    ...
    expect(rD.thresholdReduce.hay ?? 0).toBe(0);
  ...
  it("over-hired clamped to full-slot", () => {
    ...
    expect(computeWorkerEffects(over).thresholdReduce.hay).toBe(3);
  });
