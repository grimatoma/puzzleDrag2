...
function makeGridWith(items) {
  // items: array of {key, count}; fill rest with 'tile_special_dirt'
  const grid = Array.from({ length: 2 }, () =>
    Array.from({ length: 4 }, () => ({ key: "tile_special_dirt" })),
  );
  let idx = 0;
...
describe("10.6 — USE_TOOL scythe_full", () => {
  ...
  it("does NOT consume a turn", () => {
    const grid = makeGridWith([{ key: "tile_grain_wheat", count: 3 }]);
    const s0 = {
      ...createInitialState(),
      grid,
      tools: { ...createInitialState().tools, scythe_full: 1 },
