...
describe("Phase 5.5 — Research timer (cumulative, global)", () => {
  ...
  it("B: chain of prerequisite increments progress and accumulates", () => {
    const a1 = rootReducer(base, { type: "CHAIN_COMMIT", payload: { key: "tile_grass_hay", length: 8 } });
    expect(a1.tileCollection.researchProgress.tile_grass_spiky).toBe(8);
    const a2 = rootReducer(a1, { type: "CHAIN_COMMIT", payload: { key: "tile_grass_hay", length: 5 } });
  ...
  it("C: unrelated chain does not move unrelated counters", () => {
    const a2 = rootReducer(
      rootReducer(base, { type: "CHAIN_COMMIT", payload: { key: "tile_grass_hay", length: 8 } }),
      { type: "CHAIN_COMMIT", payload: { key: "tile_grass_hay", length: 5 } }
    );
    ...
    const c1 = rootReducer(a2, { type: "CHAIN_COMMIT", payload: { key: "tile_veg_carrot", length: 12 } });
  ...
  it("D: reaching threshold flips discovered + queues bubble", () => {
    ...
    const d1 = rootReducer(d0, { type: "CHAIN_COMMIT", payload: { key: "tile_grass_hay", length: 5 } });
  ...
  it("E: already-discovered tile type is a no-op (no progress drift, no re-fire)", () => {
    ...
    const e1 = rootReducer(d1Clean, { type: "CHAIN_COMMIT", payload: { key: "tile_grass_hay", length: 9 } });
