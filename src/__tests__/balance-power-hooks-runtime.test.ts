...
describe("Power hooks at runtime", () => {
  let snap;
  beforeEach(() => { snap = snapshotEffects("tile_grass_grass"); });
  afterEach(() => { restoreEffects("tile_grass_grass", snap); });
  ...
  it("free_turn_after_n grants 1 free move only when chain meets threshold", () => {
    ...
    const sShort = rootReducer(s0, {
      type: "CHAIN_COLLECTED",
      payload: { key: "tile_grass_grass", gained: 4, chainLength: 4, upgrades: 0, value: 1 },
    });
    expect(sShort.tileCollection?.freeMoves ?? 0).toBe(0);

    const sLong = rootReducer(s0, {
      type: "CHAIN_COLLECTED",
      payload: { key: "tile_grass_grass", gained: 6, chainLength: 6, upgrades: 0, value: 1 },
    });
