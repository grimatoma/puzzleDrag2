...
describe("decorations slice — coverage gaps", () => {
  const baseState = (over: Record<string, unknown> = {}) =>
    mergeTestState({
      coins: 1000,
      inventory: { home: { tile_grass_grass: 50, tile_mine_stone: 50, plank: 50, berry: 50 } },
      influence: 0,
      built: { decorations: {} },
      ...over,
    });
  ...
  it("rejects when inventory resource is short", () => {
    const s0 = baseState({ inventory: { home: { tile_grass_grass: 0 } } });
    const s1 = decoReduce(s0, {
      type: "BUILD_DECORATION",
      payload: { id: "violet_bed" },
    } as Action);
  ...
  it("violet_bed: deducts coins + hay, +20 influence, increments built count", () => {
    ...
    expect(inv(s1).tile_grass_grass).toBe(50 - def.cost.tile_grass_grass);
  ...
  it("missing built / inventory slices fall back gracefully", () => {
    const s0 = unsafeGameState({ coins: 500, inventory: { home: { tile_grass_grass: 10 } } });
    const s1 = decoReduce(s0, {
      type: "BUILD_DECORATION",
      payload: { id: "violet_bed" },
    } as Action);
