// Seasonal art for the FIR tree board tile (`tile_tree_fir`).
// File: src/textures/seasonal/tree/fir.ts (category TREE).
//
// A conical EVERGREEN fir: a triangular tree of 3–4 stacked dark-green needle
// tiers ("skirts" that widen downward) on a short trunk, narrow base. Because
// it is evergreen, the SILHOUETTE is identical in every season — foliage stays
// FULL all four seasons (leafDensity high year-round). Only the needle COLOUR,
// the winter SNOW LOAD + frost, the light wash, and the pad dressing change.
// The trunk keeps the tree's identity (Tree framing note).
//
...
//   anim(season)     = micro-motion + paint(ctx, SP[season], bobAt(t), sway…)
...
// `t` seconds), seamless, and subtle; the subject bob is 0 at t=0 with zero
// velocity (A*(1-cos)) so anim(t=0) === draw. Dressing motion is additive.
...
function bobAt(t: number, A = 0.55, w = 1.2): number {
...
const SP: Record<"Spring" | "Summer" | "Autumn" | "Winter", P> = {
  // Spring — bright new needle tips at branch ends; dewy lime pad + blossom.
  Spring: {
    needleDark: [28, 74, 44],
    needleMid: [44, 108, 60],
    needleLight: [98, 156, 84],
    tipHighlight: [176, 214, 120], // fresh lime new growth at the tips
    trunk: [118, 84, 50],
    trunkShade: [86, 60, 36],
    padGrass: [126, 200, 88],
    soil: [104, 78, 44],
    outlineTint: [22, 46, 30],
    lightTint: [214, 238, 230], // cool-bright
    leafDensity: 1.0,
    tipFreshAmt: 0.9,
    coneAmt: 0,
    frostAmt: 0,
  ...
  // Summer — PEAK: deep saturated evergreen green; warm light, strong shadow.
  Summer: {
    needleDark: [20, 66, 38],
    needleMid: [30, 98, 50],
    needleLight: [78, 146, 70],
    tipHighlight: [104, 166, 84],
    trunk: [110, 78, 46],
    trunkShade: [78, 54, 32],
    padGrass: [70, 158, 64],
    soil: [96, 70, 40],
    outlineTint: [16, 44, 26],
  ...
  // Autumn — dustier, slightly olive evergreen green; a couple of small brown
  // cones; olive-tan pad with a few fallen leaves.
  Autumn: {
    needleDark: [40, 70, 44],
    needleMid: [62, 100, 56],
    needleLight: [108, 144, 80],
    tipHighlight: [122, 144, 82],
    trunk: [108, 76, 44],
    trunkShade: [78, 54, 30],
    padGrass: [150, 144, 86],
    soil: [110, 80, 44],
    outlineTint: [34, 50, 30],
  ...
  // beneath the thick snow load. Snowy pad + frost. NO white-out.
  Winter: {
    needleDark: [34, 64, 50],
    needleMid: [46, 88, 60],
    ...
    coneAmt: 0,
    frostAmt: 0.8,
    snowLoadAmt: 0.92, // thick snow resting on the tiers
    padSnowAmt: 1.0,
    blossomAmt: 0,
    fallenLeafAmt: 0,
    lightStrength: 0.2,
...
function needleTier(
  ...
  const tierPath = (insetW: number, insetD: number): void => {
    ...
    for (let i = 0; i <= pts; i++) {
      ...
      const pointY = i % 1 === 0 ? dip : dip; // keep simple, scalloped via quad
  ...
  // 5) fresh new needle tips at the branch ends (spring) — light lime dots.
  if (p.tipFreshAmt > 0.02) {
    ctx.fillStyle = rgb(p.tipHighlight, 0.9 * p.tipFreshAmt);
    for (let i = 0; i <= rim; i++) {
      const f = i / rim;
      const x = lerp(-half + 1, half - 1, f) + s;
      const droop = botY - Math.sin(f * Math.PI) * depth * 0.2;
      ctx.beginPath();
      ctx.arc(x - s * 0.1, droop, 1.0, 0, Math.PI * 2);
      ctx.fill();
    }
  }
}
...
// shows beneath; the snow caps the top edge of each skirt and the apex.
function tierSnow(
  ctx: CanvasRenderingContext2D,
  cy: number,
  half: number,
  ...
  for (let i = 0; i <= lumps; i++) {
    ...
    const lumpY = topY + 2.6 + Math.sin(f * Math.PI) * depth * 0.5 * a;
...
// Spring: gentle bough sway + breathing bob; dew shimmer flecks.
function animSpring(ctx: CanvasRenderingContext2D, t: number): void {
  const bob = bobAt(t, 0.5, 1.3); // 0 at t=0
  const sway = Math.sin(t * 1.3) * 1.0;
  paint(ctx, SP.Spring, bob, sway);

  // Dew shimmer — soft pulsing glints riding the new needle tips (additive).
  ctx.save();
  try {
    ...
    spots.forEach(([sx, sy], i) => {
      ...
      ctx.beginPath();
      ctx.arc(sx + sway * 0.5, gy, 0.9 + g * 0.6, 0, Math.PI * 2);
      ctx.fill();
    });
  } catch {
    /* never throw */
...
// Summer: gentle sway + needle shimmer (a faint travelling sheen on the tiers).
function animSummer(ctx: CanvasRenderingContext2D, t: number): void {
  const bob = bobAt(t, 0.5, 1.2);
  const sway = Math.sin(t * 1.25) * 1.1;
  paint(ctx, SP.Summer, bob, sway);

  // Needle shimmer — a soft sheen pulse that travels down the cone.
  ctx.save();
  try {
    const prog = (t * 0.35) % 1;
    const sy = -16 + prog * 28;
    const s = 0.1 + 0.14 * Math.sin(prog * Math.PI);
    ctx.fillStyle = `rgba(206,236,170,${clamp01(s)})`;
    ctx.beginPath();
...
function animWinter(ctx: CanvasRenderingContext2D, t: number): void {
  ...
  try {
    ...
    // Gentle cold sheen pulse over the snow-laden boughs / pad.
