// BOLD seasonal art for the OAK tree board tile — a louder, more legible variant
// of oak.ts for a side-by-side decision prototype.
//
// Same recognizable oak EVERY season (the trunk is constant; only the foliage
// shape changes — the recognized Tree exception). The seasons are made
...
//
// IDLE is mostly at rest, then EVERY ~6 s a clearly-noticeable BIG CANOPY SWAY
// (~13 px lean-and-return), and EVERY ~18 s a RARE special: a small bird hops
// onto a branch, looks around, then flits off (~1.3 s) — except in autumn, where
// the special is instead a GUST that sends a flurry of leaves off the canopy.
// Both the common and rare actions are driven by a deterministic clock and
// return to the rest pose with ZERO velocity at the window edges (seamless).
//
...
const SWAY_WIN = 1.0;
const SWAY_AMP = 13; // peak horizontal canopy travel in design px (BOLD)
