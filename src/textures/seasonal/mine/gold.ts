// Seasonal art for the GOLD mineral tile (`tile_mine_gold`).
//
// A chunky grey ROCK shot through with bright GOLD veins and a few embedded
// gold nuggets, sitting on a rocky/earth pad. The SAME boulder silhouette is
// drawn every season — only colours and small dressing (moss tint, fallen
// leaves, snow cap, frost, pad snow, light tint) change. Per the Mineral
// framing, seasonal change is mostly the global light/pad filter: the gold's
// own colour is LOCKED and stays bright and clearly visible all year; the grey
// rock stays grey.
//
...
interface P {
  ...
  goldLight: RGB;   // bright lit gold (LOCKED across seasons)
  goldMid: RGB;     // gold body tone (LOCKED across seasons)
  goldDark: RGB;    // shadowed gold (LOCKED across seasons)
  ...
  goldShine: number; // 0..1 specular gloss on the gold veins/nuggets
  ...
  fallenLeafAmt: number; // 0..1 fallen leaves on the rock + pad (autumn)
}
...
function lerpP(a: P, b: P, t: number): P {
  return {
    ...
    goldDark: lerpRGB(a.goldDark, b.goldDark, t),
    ...
    goldShine: lerp(a.goldShine, b.goldShine, t),
...
// warm/cool nudge only) so the gold reads bright and locked all year. Summer
// is the warm PEAK; winter is the coolest. Grey rock stays grey.

const _GOLD_L: RGB = [255, 224, 120];
const _GOLD_M: RGB = [236, 184, 56];
const _GOLD_D: RGB = [176, 122, 24];

const SP: Record<SeasonName, P> = {
  // Spring — cool-bright light; grey rock + bright gold; fresh moss at the base,
  // dewy pad.
  Spring: {
    rockLight: [186, 190, 196],
    rockMid: [140, 144, 152],
    rockDark: [88, 92, 100],
    goldLight: [252, 222, 124],
    goldMid: [232, 182, 60],
    goldDark: [172, 120, 28],
    padTop: [150, 142, 122],
    padDark: [96, 88, 72],
  ...
  // Summer — richest warm gold (PEAK); warm light, dry rocky pad, light moss.
  Summer: {
    rockLight: [192, 192, 190],
    rockMid: [146, 146, 146],
    rockDark: [92, 92, 94],
    goldLight: [255, 230, 130],
    goldMid: [240, 190, 58],
    goldDark: [180, 126, 24],
    padTop: [162, 144, 110],
    padDark: [108, 92, 64],
  ...
  Autumn: {
    rockLight: [184, 182, 178],
    rockMid: [140, 138, 136],
    rockDark: [88, 86, 86],
    goldLight: [252, 224, 122],
    goldMid: [234, 184, 56],
    goldDark: [174, 122, 24],
    padTop: [158, 142, 96],
    padDark: [110, 96, 58],
  ...
  // Winter — cool blue-grey light; snow cap + frost on the rock, snowy pad; the
  // gold veins still glint clearly underneath.
  Winter: {
    rockLight: [188, 196, 208],
    rockMid: [140, 148, 162],
    rockDark: [90, 98, 114],
    goldLight: [250, 224, 138],
    goldMid: [230, 184, 70],
    goldDark: [168, 122, 36],
    padTop: [172, 186, 204],
    padDark: [118, 134, 156],
...
/** Gold veins as polylines across the rock face (constant geometry). */
...
/** Embedded gold nuggets glinting on the surface (constant positions). */
...
function paint(ctx: CanvasRenderingContext2D, raw: P, bob: number): void {
  ...
  try {
    ...
    // moss/grass tint at the base of the rock (spring/summer)
    if (p.mossAmt > 0.01) {
      const a = p.mossAmt;
      ctx.fillStyle = rgba([110, 168, 78], 0.65 * a);
      [[-12, 18.5], [10, 18.2], [-3, 20], [5, 19.4]].forEach(([mx, my], idx) => {
        ctx.beginPath();
        ctx.ellipse(mx, my, 3.2 + (idx % 2), 1.6, 0, 0, Math.PI * 2);
        ctx.fill();
      });
      // a couple of tiny grass blades poking up at the rock foot
    ...
    // ── Subject: the gold-bearing rock (SAME silhouette every season) ───────
    ...
    // ── Gold veins shot through the rock (LOCKED colour, drawn bright) ───────
    // dark base pass then bright core, like the house dark-then-light idiom.
    VEINS.forEach((pts) => {
      ctx.strokeStyle = rgb(p.goldDark);
      ctx.lineWidth = 3;
      ctx.beginPath();
      ...
      ctx.strokeStyle = rgba(p.goldLight, 0.85);
      ctx.lineWidth = 0.8;
      ctx.beginPath();
    ...
    // ── Embedded gold nuggets glinting on the surface (LOCKED colour) ───────
    NUGGETS.forEach(([nx, ny, nr]) => {
      const ng = ctx.createRadialGradient(nx - nr * 0.4, ny - nr * 0.4 + bob, nr * 0.2, nx, ny + bob, nr);
      ng.addColorStop(0, rgb(p.goldLight));
      ng.addColorStop(0.6, rgb(p.goldMid));
      ng.addColorStop(1, rgb(p.goldDark));
      ctx.fillStyle = ng;
      ctx.beginPath();
      ...
      ctx.lineWidth = 0.7;
      ctx.stroke();
      // small specular glint (scaled by goldShine; the gold stays bright)
      ctx.fillStyle = rgba([255, 255, 255], 0.4 + 0.45 * p.goldShine);
      ctx.beginPath();
...
function anim(season: SeasonName): (ctx: CanvasRenderingContext2D, t: number) => void {
  return (ctx, t) => {
    ...
    try {
      ...
      // Shared: a soft specular GLINT travels along the gold veins/nuggets —
      ...
      if (season === "Spring") {
        ...
        // brightest/warmest gold glint already handled above; add a faint
        // warm bloom on the largest nugget shoulder
        const s = 0.16 + 0.2 * (0.5 + 0.5 * Math.sin(t * 1.1));
        ctx.fillStyle = `rgba(255,236,170,${s})`;
        ctx.beginPath();
