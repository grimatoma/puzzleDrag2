// Seasonal art for the CATTLE LONGHORN board tile (`tile_cattle_longhorn`).
//
// A SINGLE parameterized paint renders the whole tile (grass pad + the one
// tan longhorn steer with its signature very-wide sweeping horns + seasonal
// dressing) from a tweenable parameter set `P`. The four seasons are four `P`
// presets; the three forward transitions lerp EVERY field of `P` through a
// quintic smoothstep, so transition(0) === the from-season still and
// transition(1) === the to-season still — no snap.
//
// PALETTE LOCK: the longhorn is ALWAYS the same tan/russet-bodied steer with a
// white face blaze, dark hooves, a tail, and a pair of VERY WIDE spreading
// horns (the signature long horns reaching out to both sides). The SILHOUETTE
// and the tan/russet IDENTITY colours are constant across all seasons. Only the
// COAT thickness (`coatVolume` — sleek in spring → thicker in winter), the pad
// colour, the light wash, and dressing (blossom, fallen leaves, frost,
// back-snow, breath-fog, frosted horns) change. The longhorn is never
...
// coat volume + pad + light + dressing amounts.
...
interface P {
  ...
  coatVolume: number; // 0..1 coat thickness (sleek → thick)
  frostAmt: number; // 0..1 frost sparkle on the coat
  hornFrostAmt: number; // 0..1 frost rime on the wide horns (winter)
...
// Four season presets. The longhorn stays the SAME tan, white-blazed,
// wide-horned steer; only coat volume + pad + light + dressing differ.
const SP: Record<SeasonName, P> = {
  // Spring — sleek tan coat, dewy lime pad + blossom, cool-bright light.
  Spring: {
    hideLight: [214, 162, 100],
    hideMid: [184, 126, 70],
  ...
  // Summer — normal tan coat (PEAK), saturated mid-green pad, warm light.
  Summer: {
    hideLight: [222, 162, 92],
    hideMid: [192, 124, 60],
  ...
  // Autumn — slightly fuller tan coat, olive-tan browning pad, fallen leaves.
  Autumn: {
    hideLight: [206, 144, 82],
    hideMid: [176, 110, 54],
    hideShadow: [134, 78, 38],
    blaze: [244, 238, 226],
  ...
  // Winter — thicker tan coat + frosted horns + back-snow + breath fog, snowy
  // pad, cool light. The tan hide stays clearly visible under the frost.
  Winter: {
...
function lerpP(a: P, b: P, t: number): P {
  return {
    ...
    hornFrostAmt: lerp(a.hornFrostAmt, b.hornFrostAmt, t),
...
function clampP(p: P): P {
  return {
    ...
    hornFrostAmt: clamp01(p.hornFrostAmt),
...
// lock); winter only adds a frost rime over them.
function paintHorns(ctx: CanvasRenderingContext2D, p: P, hx: number, hy: number): void {
  for (const side of [-1, 1] as const) {
    // base near the crown, sweeping far out to the side and tipping up
    const x0 = hx + side * 2.2;
    const y0 = hy - 4.4;
    const cx = hx + side * 12.5; // far reach
    const cy = hy - 5.6;
    const x1 = hx + side * 17.5; // very wide tip
    ...
    ctx.strokeStyle = rgb(p.hornLight);
    ctx.lineWidth = 1.2;
    ctx.beginPath();
    ...
    // frost rime on the horns (winter) — cool speckle along the sweep
    if (p.hornFrostAmt > 0.001) {
      ctx.fillStyle = rgb([236, 246, 255], 0.85 * p.hornFrostAmt);
      for (let k = 0; k <= 5; k++) {
        const tt = k / 5;
        ...
        ctx.beginPath();
        ctx.arc(bxp, byp - 0.4, 0.7, 0, Math.PI * 2);
        ctx.fill();
      }
    }
    ctx.lineCap = "butt";
...
// the whole longhorn, standing front-¾ turned ~15-20° toward lower-left
function paintLonghorn(ctx: CanvasRenderingContext2D, p: P, bob: number, headDip = 0): void {
  ...
  // thicker winter coat fringe: a few soft tufts along the lower/back edge,
  ...
  if (p.backSnowAmt > 0.001) {
    ...
    ctx.beginPath();
    ctx.ellipse(bx, by - 7.2, 10 * (0.9 + vol * 0.2), 3.2, -0.06, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = rgb([255, 255, 255], 0.85 * p.backSnowAmt);
    for (const [dx, dy] of [[-6, -8.0], [0, -8.6], [6, -7.8]] as const) {
      ctx.beginPath();
  ...
  //    WIDE horns, ears, dark muzzle. A small head dip lowers it slightly. ────
  const hx = bx - 14;
  const hy = by + 3 + headDip;

  // the signature WIDE horns (behind the crown), constant width every season
  paintHorns(ctx, p, hx, hy);

  // ears (tan outer + lighter inner), set wide, behind the head
  for (const side of [-1, 1] as const) {
    ctx.save();
  ...
  ctx.fillStyle = rgb(p.hideLight, 0.55);
  ctx.beginPath();
  ...
  if (p.breathFogAmt > 0.001) {
    ...
    ctx.beginPath();
    ctx.ellipse(hx - 5.5, hy + 4.6, 3.2, 2.2, 0.2, 0, Math.PI * 2);
    ctx.fill();
  }
}
...
function paint(ctx: CanvasRenderingContext2D, pIn: P, bob: number, headDip = 0): void {
  ...
  try {
    ctx.lineJoin = "round";
    paintPad(ctx, p);
    paintLonghorn(ctx, p, bob, headDip);
...
function anim(season: SeasonName) {
  return (ctx: CanvasRenderingContext2D, t: number): void => {
    ...
    try {
      ...
      ctx.save();
      ctx.translate(hx - 5.4, hy - 1.6);
      ctx.rotate(-0.85 + flick * 0.6);
      ctx.fillStyle = rgb(p.outline);
      ctx.beginPath();
      ctx.ellipse(0, 0, 3.2, 1.8, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.fillStyle = rgb(p.hideShadow);
      ctx.beginPath();
      ctx.ellipse(0.3, 0.1, 1.4, 0.7, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
      ...
      if (season === "Spring") {
        ...
        // soft coat sheen sweeping across the tan hide
        const s = 0.5 + 0.5 * Math.sin(t * 1.1);
        const sx = bx - 10 + s * 20;
        ctx.save();
        ctx.globalCompositeOperation = "soft-light";
        ctx.fillStyle = rgb([255, 248, 224], 0.32);
        ctx.beginPath();
        ...
        ctx.beginPath();
        ctx.ellipse(hx - reach - 1, hy + 4.6, 2.6 + breathe * 1.8, 1.8 + breathe * 1.2, 0.2, 0, Math.PI * 2);
        ctx.fill();
