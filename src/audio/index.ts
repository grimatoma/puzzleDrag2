...
// Augment Window to support the non-standard webkitAudioContext present in
// some older browsers. DOM types do not include it.
declare global {
  interface Window {
...
export function setEnabled(next: Partial<EnabledState>): void {
 ...
 */
function playStep(step: SoundStep, baseDelay: number): void {
  const { freq, freqEnd, dur, type, gain = 0.06, delay = 0 } = step;
  const c = getCtx();
  ...
  if (freqEnd !== undefined) osc.frequency.exponentialRampToValueAtTime(freqEnd, t + durSec);
...
export interface PlayOptions {
  /** Multiplicative pitch shift (0.5–2.5). Defaults to 1. */
  pitch?: number;
}
 ...
 */
export function play(name: string, opts: PlayOptions = {}): void {
  ...
  def.steps.forEach((step) => {
    ...
    playStep(
      {
        ...step,
        freq: step.freq * pitch,
        freqEnd: step.freqEnd !== undefined ? step.freqEnd * pitch : undefined,
      },
      0
    );
  });
