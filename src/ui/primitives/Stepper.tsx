...
interface SizeSpec { btn: number; value: string; chip: string }
const SIZES: Record<StepperSize, SizeSpec> = {
...
export default function Stepper({
  ...
  const holdRef = useRef<HoldRef>({ timeout: null, interval: null });
  ...
  const handlePreset = (p: number | "max") => {
    if (disabled || !onChange) return;
    if (p === "max") {
