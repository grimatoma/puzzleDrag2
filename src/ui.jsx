...
export function Hud({ state }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 bg-[#5b3b20] border-b-2 border-[#2a1d0f] text-[#6a4b31] flex-wrap">
      <Pill>
      ...
      <div className="ml-auto flex items-center gap-2">
        <div className="bg-[#f6efe0] border-2 border-[#b28b62] rounded-full h-[28px] w-[120px] relative overflow-hidden">
          <div className="h-full bg-gradient-to-r from-[#ff8b25] to-[#ffb347] transition-[width] duration-300" style={{ width: `${xpPct}%` }} />
          <div className="absolute inset-0 grid place-items-center text-[11px] font-bold text-white" style={{ textShadow: "0 1px 2px rgba(0,0,0,.4)" }}>
            {xp} / {xpNeed}
          </div>
        </div>
        <div className="w-10 h-10 rounded-full bg-[#bb3b2f] border-[3px] border-[#ffe2a3] grid place-items-center text-white font-bold text-[18px]">
          {level}
        </div>
      </div>
    </div>
...
function SeasonBar({ season, turnsUsed }) {
  return (
    <div className="bg-[#faf0dd] border-2 border-[#b28b62] rounded-full px-3 py-1 flex items-center gap-2 min-w-[280px] flex-1 max-w-[480px]">
      <div className="text-[#6a4b31] font-bold text-[13px] whitespace-nowrap">{season.name}</div>
      <div className="flex gap-1 flex-1 justify-center">
        {Array.from({ length: MAX_TURNS }).map((_, i) => {
          ...
          return (
            <div
              key={i}
              className={`w-3.5 h-3.5 rounded-full border ${filled ? "border-transparent" : "border-[#b28b62]"} transition-all`}
              style={{
...
function Section({ title, titleColor = "#f8e7c6", children }) {
  return (
    <div className="flex flex-col gap-2 min-h-0">
      <div className="font-bold text-[15px] tracking-wide" style={{ color: titleColor }}>{title}</div>
...
export function BottomNav({ view, onChange }) {
  ...
  return (
    <div className="bg-[#2b2218]/95 border-2 border-[#f7e2b6] rounded-2xl p-1.5 flex gap-1.5 shadow-2xl">
      {items.map((it) => (
        <button
          key={it.key}
          onClick={() => onChange(it.key)}
          className={`px-4 py-1.5 rounded-xl text-[13px] font-bold transition-colors ${view === it.key ? "bg-[#d6612a] text-white" : "bg-transparent text-[#f7e2b6] hover:bg-white/10"}`}
        >
          {it.label}
        </button>
      ))}
...
export function TownView({ state, dispatch }) {
  return (
    ...
    >
      ...
      {/* Header */}
      <div className="absolute top-3 left-4 font-bold text-[22px] text-[#3a2715]">Hearthwood Vale</div>
      <div className="absolute top-3 right-4 bg-white/85 px-3 py-1.5 rounded-full font-bold text-[#3a2715]">◉ {state.coins.toLocaleString()}</div>
...
export function SeasonModal({ state, dispatch }) {
  ...
  return (
    <div className="absolute inset-0 bg-black/55 grid place-items-center z-50 animate-fadein">
      <div className="bg-[#f4ecd8] border-[4px] border-[#b28b62] rounded-[20px] px-9 py-7 min-w-[420px] max-w-[560px] text-center shadow-2xl">
        <div className="text-[56px] leading-none">{nextSeason.icon === "flower" ? "✿" : nextSeason.icon === "sun" ? "☀" : nextSeason.icon === "leaf" ? "🍂" : "❄"}</div>
        <h2 className="font-bold text-[28px] text-[#744d2e] mt-2 mb-1">{prevSeason.name} ends</h2>
        <p className="italic text-[#6a4b31]">The wind shifts. {nextSeason.name} arrives in Hearthwood Vale.</p>
...
function Stat({ v, l }) {
  return (
    <div>
      <div className="font-bold text-[24px] text-[#a8431a]">{v}</div>
      <div className="uppercase tracking-widest text-[10px] text-[#8a785e]">{l}</div>
    </div>
  );
}
...
export function NpcBubble({ bubble, dispatch }) {
  ...
  if (!npc) return null;
  return (
    <div className="absolute bottom-20 left-1/2 -translate-x-1/2 bg-[#f4ecd8] border-[3px] border-[#5a3a20] rounded-2xl px-4 py-3 max-w-[460px] shadow-2xl z-40 animate-bubblein">
