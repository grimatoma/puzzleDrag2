// Core tile catalog (TILE_TYPES, CATEGORIES, colors, lookups). Relocated out of
// src/features/tileCollection/ — it is the canonical game catalog imported by
// ~26 modules across game/, config/, state, GameScene, and the wiki, not a
// feature-private module. (Health review #5.) The old feature path re-exports
// from here for back-compat. This module is a leaf: it imports only two pure
// config leaves below, so it never participates in an import cycle.
