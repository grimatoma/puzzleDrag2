...
// settlement · N◉" button for a visited-but-unfounded settleable zone. A
// parallel CTA also lives in the Town view (src/ui/Town.jsx) so the player
// can found from either surface, and founding is now enforced reducer-side
// (BUILD / FARM/ENTER / EXPEDITION/DEPART refuse at unfounded zones).
//
...
function FoundSettlementControl({ node, visitedSet, state, dispatch, fullWidth }) {
  ...
  return (
    <>
      ...
      {pickerOpen && <BiomePicker node={node} type={type} cost={cost} dispatch={dispatch} onClose={() => setPickerOpen(false)} />}
    </>
  );
}
