...
function TermCard({ term, choiceLog, flags, onOpen }: TermCardProps) {
  ...
  return (
    ...
    >
      ...
      <Pill tone={tone} size="sm" leading={undefined} trailing={undefined} anchor={undefined}>{label}</Pill>
    </button>
  );
}
...
function TermDialog({ term, choiceLog, flags, onClose }: TermDialogProps) {
  ...
  const state = deriveTermState(term, choiceLog, flags);
  return (
    <ParchmentDialog open onClose={onClose} size="md" ariaLabel="Term details">
      ...
      <ParchmentDialog.Body>
        <div className="flex items-center gap-2 mb-3">
          <Pill tone={statePillTone(state)} size="sm" leading={undefined} trailing={undefined} anchor={undefined}>{statePillLabel(state)}</Pill>
        </div>
      ...
      <ParchmentDialog.Actions sticky>
        <Button tone="iron" variant="outline" onClick={onClose} leading={undefined}>Close</Button>
