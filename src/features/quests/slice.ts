...
export function seedQuestIdSeq(savedDailies: any) {
...
export const initial = {
  ...
  almanacXp: 0,
  almanacTier: 0,
  almanacClaimed: [] as any[],
};

function progressQuests(dailies: any, key: any, amount: any) {
  return dailies.map((q: any) => {
...
export function reduce(state: any, action: any) {
  switch (action.type) {
    ...
    case "QUESTS/CLAIM_QUEST": {
      ...
      if ((state.quests || []).some((q: any) => q.id === id)) {
      ...
      const q = (state.dailies || []).find((x: any) => x.id === id);
      if (!q || !q.done || q.claimed) return state;
      const dailies = (state.dailies || []).map((x: any) =>
    ...
    case "CHAIN_COLLECTED": {
      ...
      if (Array.isArray(chainOnly) && chainOnly.length > 0 && chainOnly.every((t: any) => t.key === "rat")) {
        return state;
      }
    ...
    case "TURN_IN_ORDER": {
      const order = (state.orders || []).find((o: any) => o.id === action.id);
      ...
      const newQuests = (state.quests || []).map((q: any) => tickQuest(q, orderEvent));
      return { ...state, dailies, quests: newQuests };
    }
    case "BUILD": {
    ...
    case "CRAFTING/CRAFT_RECIPE": {
      ...
      const newQuests = (state.quests || []).map((q: any) => tickQuest(q, craftEvent));
      return { ...state, dailies, quests: newQuests };
    }
    case "CLOSE_SEASON": {
