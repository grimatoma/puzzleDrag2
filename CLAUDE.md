...
- **Headroom context compression (why `ANTHROPIC_BASE_URL` points at localhost).** `.claude/settings.json` sets `ANTHROPIC_BASE_URL=http://127.0.0.1:8787` and routes every Claude Code session on this repo through [Headroom](https://github.com/chopratejas/headroom), a **local** context-compression proxy that shrinks large tool outputs / file listings / search results before they reach the model (forwarding upstream to `api.anthropic.com`). This is intentional infra, **not** a leaked dev override — don't delete it to "fix" routing. The proxy is brought up by `.claude/hooks/headroom-bootstrap.sh`: a `SessionStart` hook (`bootstrap` mode) installs the `headroom` CLI into a self-owned venv (`~/.cache/headroom-bootstrap/venv`) on fresh web containers and blocks until the proxy is healthy; a `PreToolUse` Bash hook (`ensure` mode) is a near-instant health check that restarts it if it died. Telemetry is disabled (`--no-telemetry`); the proxy is `127.0.0.1`-only. Diagnostics: `~/.cache/headroom-bootstrap/bootstrap.log` and `curl -s 127.0.0.1:8787/stats`. **To opt out** for a session: remove the `env`/headroom-hook blocks from `.claude/settings.json`, or `kill` the `headroom proxy` process. First boot adds a one-time ~60–75s install to session start (subsequent boots in the same container are instant).
