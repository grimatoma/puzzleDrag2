...
**Touch / input gotcha (the one that bit us).** `project.godot` enables **both** `pointing/emulate_mouse_from_touch` **and** `pointing/emulate_touch_from_mouse` ("treat touch as mouse so one drag path serves both"). The cost: one physical drag arrives as **two** events — a real `InputEventScreenDrag` *and* a synthesized `InputEventMouseMotion`. Any handler that reacts to both will **double-count** the gesture. This is exactly the "scroll moves at 2× my finger" bug — fixed by routing drags through the mouse path only (`drag_with_touch = false` in `UiKit.make_vscroll()`, the single factory every list/modal scroll goes through). The same doubling hits **discrete taps**, not just drags: a single tap fires both a `MouseButton` press and an emulated `ScreenTouch` press, so a `gui_input` handler reacting to both runs **twice** — a toggle cancels itself (expand→collapse) and a modal scrim can dismiss the *same* tap that just opened the modal (the "settings button does nothing on a touchpad" bug — only repros with a fast tap, where press+release coalesce so the emulated touch press lands after the modal is already up). Fixed by making tap handlers mouse-path only too (`UiKit.wire_backdrop_dismiss` / `make_expandable_chip`; `Hud._slot_gui_input` already did this). When adding **any** new drag, gesture, **or tap** handling in the port, listen to **one** event type, not both — the mouse path, since `emulate_mouse_from_touch` keeps real touchscreen input flowing through it.
