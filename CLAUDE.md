...
- **Use the `html-docs` skill** (`.claude/skills/html-docs/SKILL.md`) whenever you save a doc into the repo. It ships a ready-to-copy `template.html` (sticky-TOC layout, callouts, tables, inline-SVG diagram, collapsible `<details>`, filter/tab JS, scroll-spy nav, and print/mobile/reduced-motion media queries) plus a `design-principles.md` reference. Start from the template and retheme it; don't hand-roll a doc from scratch. The skill is project-agnostic — copy the folder into other repos to carry the convention forward.
