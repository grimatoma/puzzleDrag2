...
(function () {
  ...
  function paintGround(g, Z, lots, tier) {
    ...
    const cleared = (x, y) => nearRoad(x, y, 70) || onLot(x, y, 20) || inPlaza(x, y, 10) || nearLM(x, y) || nearSpan(x, y, 22);
    ...
    for (let ty = 0; ty < ROWS; ty++) for (let tx = 0; tx < COLS; tx++) {
      ...
      if (water) { if (cl) { g.fillStyle = shade(P.ground, .08 + (rnd() < .5 ? .06 : 0)); g.fillRect(tx * TILE, ty * TILE, TILE, TILE); } }
      else { g.fillStyle = cl ? shade(P.ground, .10 + (rnd() < .3 ? .06 : 0)) : (P.frontierFill || shade(P.ground, -0.2)); g.fillRect(tx * TILE, ty * TILE, TILE, TILE); }
    }
    // soft tonal blotches
  ...
  function drawFeature(g, ft, P, tier) {
    ...
    else if (ft.kind === "lava") { const stroke = (col, w) => { g.strokeStyle = col; g.lineWidth = w; g.lineCap = "round"; g.lineJoin = "round"; g.beginPath(); g.moveTo(ft.pts[0][0], ft.pts[0][1]); for (let i = 1; i < ft.pts.length; i++) g.lineTo(ft.pts[i][0], ft.pts[i][1]); g.stroke(); };
      g.save(); g.shadowColor = P.glow; g.shadowBlur = 26; stroke(P.surface, ft.w || 22); g.restore();
      stroke(P.glow, (ft.w || 22) * 0.55); g.globalAlpha = 0.7; stroke("#fff3d0", (ft.w || 22) * 0.22); g.globalAlpha = 1; }
  ...
  function drawRoads(g, Z, tier) {
    const P = Z.palette;
    for (const r of Z.roads) { if (r.tier > tier) continue; const p = segPts(r); const col = r.col || (r.kind === "boardwalk" ? P.path : (tier >= 3 ? (P.cobble || shade(P.path, .12)) : P.path));
