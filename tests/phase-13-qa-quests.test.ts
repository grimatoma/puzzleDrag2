...
describe("Fix 4 — deterministic 6-slot quests", () => {
  ...
  it("CHAIN_COLLECTED ticks collect quests in state.quests", () => {
    const s = {
      ...fresh(),
      quests: [{
        id: "q1", template: "collect_hay", category: "collect", key: "tile_grass_hay",
        target: 30, progress: 0, claimed: false, reward: { coins: 60, xp: 20 },
      }],
    };
    const next = rootReducer(s, {
      type: "CHAIN_COLLECTED",
      payload: { key: "tile_grass_hay", gained: 10, upgrades: 0, value: 1, chainLength: 5, noTurn: false },
    });
  ...
  it("CLAIM_QUEST marks quest claimed in state.quests", () => {
    const s = {
      ...fresh(),
      quests: [{
        id: "qclaim1", template: "collect_hay", category: "collect", key: "tile_grass_hay",
