...
describe("XP per source — CHAIN_COLLECTED (1 XP/chain)", () => {
  it("one chain awards exactly 1 almanac XP", () => {
    ...
    const next = rootReducer(s2, {
      ...
      payload: { key: "tile_grass_grass", gained: 5, upgrades: 0, value: 1, chainLength: 5, noTurn: false },
    });
  ...
  it("two chains = 2 almanac XP", () => {
    let s = fresh();
    const payload = { key: "tile_grass_grass", gained: 3, upgrades: 0, value: 1, chainLength: 3, noTurn: false };
...
describe("XP per source — BUILD (10 XP)", () => {
  it("building awards 10 almanac XP", () => {
    ...
    const rich = { ...s, coins: 99999, inventory: { home: { tile_grass_grass: 999, tile_tree_oak: 999, tile_mine_stone: 999, tile_mine_iron_ore: 999, grain: 999 } } };
...
describe("XP per source — CLAIM_QUEST (20 XP)", () => {
  it("claiming a quest awards 20 almanac XP", () => {
    const s = {
      ...fresh(),
      quests: [{
        id: "tq1", template: "collect_hay", category: "collect", key: "tile_grass_grass",
